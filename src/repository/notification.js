...
const getById = async (id) => {
  const notification = await getKnex()(tables.order_notification)
...
const updateById = async (
  id,
  { notification_date, CUSTOMER_supplier_id, ORDER_order_id, status, message }
) => {
  try {
...
const updateByArray = async (array) => {
  try {
    ...
    return await getKnex()(tables.order_notification)
      .select("*")
      .whereIn(`${tables.order_notification}.notification_id`, array);
  } catch (error) {
